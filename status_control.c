...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/03/12 15:17:20 by hvahib            #+#    #+#             */
/*   Updated: 2025/03/12 15:19:44 by hvahib           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	hunger_check(t_philo *philo)
{
	...
	if (time - philo->last_meal >= philo->t_die && philo->is_eating == 0)
	{
...
int	dead_check(t_philo *philos)
{
	...
	while (i < philos->n_philos)
	{
		...
		{
			if (log_printing(&philos[i], "died", 1))
				return (1);
			if (pthread_mutex_lock(philos->lock_dead))
				return (error_out("Error: Mutex Lock Failed!"));
			*(philos->dead) = 1;
